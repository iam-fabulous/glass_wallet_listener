...
class BackendService {
    async sendTransactionConfirmation(payload) {
        logger_1.logger.info(`Sending transaction confirmation for degree ${payload.transactionDigest} to backend...`);
        try {
            ...
            logger_1.logger.info(`Java Backend for ${payload.transactionDigest}: Status ${response.status}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to send confirmation for ${payload.transactionDigest} to Java backend: `, error.message);
            if (error.response) {
